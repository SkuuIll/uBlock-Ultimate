...
const initializeTabs = async ( ) => {
    const manifest = typeof browser !== 'undefined' ? browser.runtime.getManifest() : chrome.runtime.getManifest();
    ...
    if ( vAPI.canWASM && µb.hiddenSettings.disableWebAssembly !== true ) {
        const wasmModuleFetcher = function(path) {
            return fetch(`${path}.wasm`, { mode: 'same-origin' }).then(
                ...
                console.warn('[uBR] start: WASM module fetch failed', reason);
            });
        };
...
(typeof browser !== 'undefined' ? browser : chrome).runtime.onUpdateAvailable.addListener(details => {
