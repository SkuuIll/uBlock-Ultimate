...
const fromExtendedFilter = function(details) {
    ...
    if (match === null) {
        self.postMessage({ id: details.id, response: Object.create(null) });
        return;
    }
    ...
    const needleMatches = (details.needle || selector).match(/\w+|\*/g);
    if (needleMatches === null || needleMatches.length === 0) {
        self.postMessage({ id: details.id, response: Object.create(null) });
        return;
    }
    const needle = needleMatches.reduce((a, b) => {
    ...
    const regexTargetMatchesURL = target => {
        ...
        return regexTargetMatchesHostname(`${target.slice(0, pathPos)}$`) &&
            (new RegExp(`^${target.slice(pathPos)}`)).test(pathname);
    };
