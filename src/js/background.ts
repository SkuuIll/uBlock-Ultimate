...
const µBlock = {  // jshint ignore:line
    ...
    canFilterResponseData: typeof browser !== 'undefined' && typeof browser.webRequest.filterResponseData === 'function',
    ...
    openNewTab(details) {
        ...
        if (details.newTab) {
            ...
            chrome.tabs.query({ active: true, currentWindow: true }, tabs => {
                if (tabs[0]?.id) {
                    chrome.tabs.update(tabs[0].id, { url, active: true }).catch(() => {});
                }
            });
        }
    },
...
// Initialize Blocker Core adapter
(async () => {
    try {
...
// Initialize Smart Cosmetic Engine
(async () => {
    try {
        const { smartEngine } = await import('../core/smart-cosmetic/engine');
        await smartEngine.init();
        const stats = smartEngine.getStats();
        console.log('[uBR] Smart Cosmetic Engine initialized:', stats);
    } catch (e) {
        console.error('[uBR] Failed to initialize Smart Cosmetic Engine:', e);
    }
})();
