...
function m3uPrune(
    ...
    const pruneSpliceoutBlock = (lines, i) => {
        ...
        if ( i < lines.length && lines[i] && lines[i].startsWith('#EXT-X-ASSET:CAID') ) {
            toLog.push(`\t${lines[i]}`);
            lines[i] = undefined; i += 1;
        }
        if ( i < lines.length && lines[i] && lines[i].startsWith('#EXT-X-SCTE35:') ) {
            toLog.push(`\t${lines[i]}`);
            lines[i] = undefined; i += 1;
        }
        if ( i < lines.length && lines[i] && lines[i].startsWith('#EXT-X-CUE-IN') ) {
            toLog.push(`\t${lines[i]}`);
            lines[i] = undefined; i += 1;
        }
        if ( i < lines.length && lines[i] && lines[i].startsWith('#EXT-X-SCTE35:') ) {
    ...
    const pruneInfBlock = (lines, i) => {
        ...
        if ( i + 1 >= lines.length || reM3u.test(lines[i+1]) === false ) { return false; }
        toLog.push('Discarding', `\t${lines[i]}, \t${lines[i+1]}`);
        lines[i] = lines[i+1] = undefined; i += 2;
        if ( i < lines.length && lines[i] && lines[i].startsWith('#EXT-X-DISCONTINUITY') ) {
            toLog.push(`\t${lines[i]}`);
            lines[i] = undefined; i += 1;
        }
        return true;
