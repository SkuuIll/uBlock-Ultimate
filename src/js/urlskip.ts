    ...
    try {
        ...
        for ( const step of steps ) {
            ...
            if ( c0 === 0x2D ) { // -
                ...
                // Safe Base64
                if ( step === '-safebase64' ) {
                    if ( (urlSkip as any).safeBase64Replacer === undefined ) {
                        (urlSkip as any).safeBase64Map = { '-': '+', '_': '/' };
                        (urlSkip as any).safeBase64Replacer = (s: string) => (urlSkip as any).safeBase64Map[s];
                    }
                    urlout = urlin.replace(/[-_]/g, (urlSkip as any).safeBase64Replacer);
