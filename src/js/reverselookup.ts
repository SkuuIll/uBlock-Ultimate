...
const workerTTLTimer = (vAPI as any).defer.create(stopWorker);
...
const initWorker = function() {
    ...
    for ( const listKey of entries.keys() ) {
        promises.push(
            (µb as any).getCompiledFilterList(listKey).then(details => {
...
const fromNetFilter = async function(rawFilter) {
    ...
    const parser = new sfp.AstFilterParser({
        ...
        nativeCssHas: (vAPI as any).webextFlavor?.env.includes('native_css_has') ?? false,
    });
