    ...
    const renderToDiv = function(vwEntry, i) {
        ...
        if (
            ...
            try {
                ...
                console.warn(
                    '[uBR] Logger received an invalid highlight regex:',
                    filter.regex,
                    error
                );
            }
...
const popupManager = (( ) => {
    ...
    const onMessage = function(ev) {
        ...
        if ( ev.data?.source !== 'uBlockUltimate' || ev.data?.type !== 'embeddedPopupClose' ) {
            return;
        }
        toggleOff();
    };
