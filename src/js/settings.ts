...
async function changeUserSettings(name, value) {
    const response = await vAPI.messaging.send('dashboard', {
        what: 'userSettings',
        name,
        value,
    ...
    if ( response?.ok !== true ) {
        throw new Error(
            response?.error || `Failed to save setting: ${name}`
        );
    }
...
async function onValueChanged(ev) {
    const input = ev.target;
    const name = dom.attr(input, 'data-setting-name');
    const previousValue = input.dataset.persistedValue ?? input.value;
    ...
    case 'largeMediaSize':
        value = Math.min(
            Math.max(Math.floor(parseInt(value, 10) || 0), 0),
            1000000
        );
        break;
    default:
    ...
    try {
        await changeUserSettings(name, value);
        input.dataset.persistedValue = String(value);
    } catch (error) {
        input.value = previousValue;
        console.error(
            '[uBR] Failed to save user setting:',
            name,
            error
        );
    } finally {
...
function onUserSettingsReceived(details) {
    ...
    const onchange = async ev => {
        ...
        try {
            await changeUserSettings(name, requestedValue);
            synchronizeDOM();
        } catch (error) {
            // Restore the previous visible value if persistence failed.
            checkbox.checked = !requestedValue;
            synchronizeDOM();
            console.error('[uBR] Failed to save user setting:', name, error);
        } finally {
    ...
    qsa$('[data-setting-type="value"]').forEach((elem) => {
        const value = details[dom.attr(elem, 'data-setting-name')];
        elem.value = value;
        elem.dataset.persistedValue = String(value);
