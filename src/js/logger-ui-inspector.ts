...
const extensionApi: any = (typeof browser !== 'undefined') ? browser : chrome;
...
const contentInspectorChannel = (( ) => {
    ...
    const start = ( ) => {
        ...
        extensionApi.webNavigation.onDOMContentLoaded.addListener(onContentLoaded);
    };

    const shutdown = ( ) => {
        extensionApi.webNavigation.onDOMContentLoaded.removeListener(onContentLoaded);
        disconnect();
        bcChannel.close();
    ...
    const connect = (tabId, frameId) => {
        disconnect();
        try {
            toContentPort = extensionApi.tabs.connect(tabId, { frameId });
