...
const allTypesBits =
    ...
    1 << (typeNameToTypeValue['popunder'] >>> TYPE_REALM_OFFSET) - 1 |
    ...
    static isLAN(ipaddr) {
        ...
        if ( c0 === 0x31 /* 1 */ ) {
            ...
            if ( ipaddr.startsWith('172.') ) {
                const dotIdx = ipaddr.indexOf('.', 4);
                const end = dotIdx !== -1 ? dotIdx : ipaddr.length;
                const v = parseInt(ipaddr.slice(4, end), 10);
                return v >= 16 && v <= 31;
            }
...
StaticNetFilteringEngine.prototype.dnrFromCompiled = function(op, context, ...args) {
    ...
    const realms = new Map([
        ...
        [ HEADERS_REALM | IMPORTANT_REALM, { type: 'block', priority: 10 } ],
...
StaticNetFilteringEngine.prototype.fromSelfie = function(selfie) {
    ...
    if ( results.every(v => v === true) === false ) { return false; }
    ...
    for ( const directive of directives ) {
        ...
        while ( i > 0 ) {
            ...
            if ( name !== undefined ) {
                ...
                catch (e) { console.warn('[uBR] static-net-filtering: decodeURIComponent failed', e); }
...
StaticNetFilteringEngine.prototype.test = function(details) {
    ...
    if ( r !== 1 ) {
        ...
        if ( fctxt.redirectURL !== undefined && this.hasQuery(fctxt) ) {
            const entries = this.filterQuery(fctxt);
            if ( entries ) {
                for ( const entry of entries ) {
                    out.push(`modified: ${entry.logData().raw}`);
                }
