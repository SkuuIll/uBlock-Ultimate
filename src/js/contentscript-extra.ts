...
class ProceduralFilterer {
    constructor(domFilterer) {
        ...
        if ( vAPI.shutdown instanceof Object ) {
            vAPI.shutdown.add(() => { this.stopMutationObserver(); });
        }
    }
    ...
    startMutationObserver() {
        ...
        this.mutationObserver = new MutationObserver(
            mutations => { this.onProceduralMutations(mutations); }
        );
    ...
    onProceduralMutations(mutations) {
        ...
        for ( const mutation of mutations ) {
            if ( mutation.type === 'attributes' ) {
                ...
                if (
                    name === this.masterToken ||
                    Array.from(this.styleTokenMap.values()).includes(name)
                ) {
                    if ( mutation.target.hasAttribute(name) ) { continue; }
                    this.commitNow();
                    return;
                }
                continue;
            ...
            if (
                this.mutationTouchesElementList(mutation.addedNodes) ||
                this.mutationTouchesElementList(mutation.removedNodes)
            ) {
                this.commitNow();
                return;
            }
    ...
    destroy() {
        ...
        if ( vAPI.domWatcher instanceof Object ) {
            vAPI.domWatcher.removeListener(this);
        }
        for ( const node of this.styledNodes ) {
            node.removeAttribute(this.masterToken);
            for ( const token of this.styleTokenMap.values() ) {
...
if ( vAPI.domFilterer instanceof Object ) {
    try {
        ...
        console.warn('[uBR] contentscript-extra: procedural startup commit failed', e);
    }
}
