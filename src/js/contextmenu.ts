...
if ( (vAPI as any).contextMenu === undefined ) {
    return {
        update: function() {}
    };
}
...
const onBlockElement = function(details: ContextMenuDetails, tab?: TabInfo): void {
    ...
    (µb as any).epickerArgs.mouse = true;
    (µb as any).elementPickerExec(tab.id, 0, `${tagName}\t${src}`);
};
...
const onBlockElementInFrame = function(details: ContextMenuDetails, tab?: TabInfo): void {
    ...
    (µb as any).epickerArgs.mouse = false;
    (µb as any).elementPickerExec(tab.id, details.frameId || 0);
};
...
const onSubscribeToList = function(details: ContextMenuDetails): void {
    ...
    const hash = (µb as any).selectedFilterLists.indexOf(url) !== -1
        ? '#subscribed'
        : '';
    (vAPI as any).tabs.open({
        url:
            `/asset-viewer.html` +
...
const onTemporarilyAllowLargeMediaElements = function(details: ContextMenuDetails, tab?: TabInfo): void {
    if ( tab === undefined ) { return; }
    const pageStore = (µb as any).pageStoreFromTabId(tab.id);
...
const update = function(tabId?: number): void {
    let newBits = 0;
    if ( (µb as any).userSettings.contextMenuEnabled ) {
        const pageStore = tabId ? (µb as any).pageStoreFromTabId(tabId) : null;
        if ( pageStore?.getNetFilteringSwitch() ) {
            if ( (µb as any).userFiltersAreEnabled() ) {
        ...
        if ( (µb as any).hiddenSettings.filterAuthorMode ) {
    ...
    (vAPI as any).contextMenu.setEntries(usedEntries, onEntryClicked);
};
...
(vAPI as any).contextMenu.onMustUpdate = async function(tabId?: number): Promise<void> {
    if ( (µb as any).userSettings.contextMenuEnabled === false ) {
        return update();
    }
    if ( tabId !== undefined ) {
        return update(tabId);
    }
    const tab = await (vAPI as any).tabs.getCurrent();
    if ( typeof tab !== 'object' || tab === null ) { return; }
    update(tab.id);
};

return { update: (vAPI as any).contextMenu.onMustUpdate };
