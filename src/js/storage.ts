        ...
        this.parsedTrustedListPrefixes =
            µb.hiddenSettings.trustedListPrefixes.split(/ +/).map(prefix => {
                ...
                try {
                    ...
                    console.warn('[uBR] storage: isTrustedList URL parse failed for prefix', prefix, e);
                }
...
µb.compilePublicSuffixList = function(content) {
    ...
    const pslSelfieData = { pslSelfie: { magic: selfie.magic, buf32: Array.from(selfie.buf32) } };
    if ( vAPI?.storage?.local?.set ) {
        vAPI.storage.local.set(pslSelfieData).catch((e: unknown) => {
            console.warn('[uBR] storage: compilePublicSuffixList vAPI.storage.local.set failed', e);
        });
    } else if ( typeof chrome !== 'undefined' && chrome.storage?.local?.set ) {
        chrome.storage.local.set(pslSelfieData).catch((e: unknown) => {
            console.warn('[uBR] storage: compilePublicSuffixList chrome.storage.local.set fallback failed', e);
        });
    }
    return io.toCache(`selfie/${this.pslAssetKey}`, selfie);
};
