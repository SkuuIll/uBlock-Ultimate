...
function normalizePslSelfie(selfie: any): any {
...
function loadPslSelfie(selfie: any): boolean {
    return selfie !== undefined &&
        selfie !== null &&
        publicSuffixList.fromSelfie(normalizePslSelfie(selfie));
}

async function loadPublicSuffixList(): Promise<boolean> {
    try {
        const stored = await chrome.storage.local.get('pslSelfie');
        if ( loadPslSelfie(stored.pslSelfie) ) { return true; }
    } catch (e) {
    ...
    try {
        ...
        if ( loadPslSelfie(result?.pslSelfie) ) { return true; }
    } catch (e) {
