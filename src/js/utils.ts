...
(µb as any).formatCount = function(count: unknown): string {
...
(µb as any).dateNowToSensibleString = function(): string {
...
(µb as any).openNewTab = function(details: TabDetails): void {
    if ( details.url.startsWith('logger-ui.html') ) {
        ...
        if ( this.userSettings.alwaysDetachLogger ) {
            ...
            const url = new URL((vAPI as any).getURL(details.url));
            url.searchParams.set('popup', '1');
            details.url = url.href;
            let popupLoggerBox;
            try {
                popupLoggerBox = JSON.parse(
                    (vAPI as any).localStorage.getItem('popupLoggerBox')
                );
            } catch (e) {
    ...
    (vAPI as any).tabs.open(details);
};

/******************************************************************************/

(µb as any).escapeRegex = function(s: string): string {
    return s.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
};

/******************************************************************************/

(µb as any).getModifiedSettings = function(edit: Record<string, unknown>, orig: Record<string, unknown> = {}): Record<string, unknown> {
...
(µb as any).settingValueFromString = function(orig: Record<string, unknown>, name: string, s: string): unknown {
