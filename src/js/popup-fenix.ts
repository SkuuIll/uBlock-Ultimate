...
const renderTooltips = function(selector) {
    for ( const [ key, details ] of tooltipTargetSelectors ) {
        ...
        const elem = qs$(key);
        if ( elem === null || elem.hasAttribute('title') === false ) { continue; }
        const text = i18n$(
            details.i18n +
...
const toggleNetFilteringSwitch = async function(ev) {
    if ( !popupData?.pageURL ) { return; }

    const requestedState = dom.cl.has(dom.body, 'off');
    ...
    try {
        ...
        console.error('[uBR] Unable to toggle filtering:', error);
        dom.cl.toggle(dom.body, 'off', popupData.netFilteringSwitch !== true);
    }
};
...
const setFirewallRule = async function(src, des, type, action, persist) {
    ...
    try {
        ...
        if (
            ...
            typeof response.firewallRules !== 'object' ||
            typeof response.hostnameDict !== 'object'
        ) {
            throw new Error(
                response?.error || 'Invalid firewall-rule response',
            );
        }
        ...
        console.error('[uBR] Unable to update firewall rule:', error);
        // Reload authoritative data for the current tab
...
const reloadTab = async function(bypassCache = false) {
    if ( typeof popupData.tabId !== 'number' ) { return; }

    // Dismiss unprocessed-requests status since the page is being reloaded
    if ( popupData.hasUnprocessedRequest === true ) {
        try {
            ...
            console.warn('[uBR] dismissUnprocessedRequest failed', e);
        }
    }
    ...
    try {
        await messaging.send('popupPanel', {
            what: 'reloadTab',
            tabId: popupData.tabId,
            url: popupData.rawURL || popupData.pageURL,
        ...
        console.error('[uBR] Unable to reload tab:', error);
    }
};

dom.on('#refresh', 'click', ev => {
    void reloadTab(ev.ctrlKey || ev.metaKey || ev.shiftKey);
});
...
const saveFirewallRules = async function() {
    try {
        ...
        console.error('[uBR] Unable to save firewall rules:', error);
        dom.cl.add(dom.body, 'needSave');
    }
};
...
const revertFirewallRules = async function() {
    ...
    const response = await messaging.send('popupPanel', {
        ...
        console.error('[uBR] Unable to revert firewall rules:', error);
        dom.cl.add(dom.body, 'needSave');
    }
};
...
const toggleHostnameSwitch = async function(ev) {
    ...
    const switchName = target.id;
    if ( !switchName ) { return; }
    if ( dom.cl.has(target, 'hnSwitchBusy') ) {
        return;
    }

    const previousState = dom.cl.has(target, 'on');
    const requestedState = !previousState;

    dom.cl.toggle(target, 'on', requestedState);
    dom.cl.add(target, 'hnSwitchBusy');
    ...
    try {
        ...
        if (
            ...
            throw new Error(response?.error || 'Invalid hostname-switch response');
        }
        ...
        if ( response.reloadRequired === true ) {
            forceReloadFlag |= 1;
        }
        ...
        dom.cl.toggle(target, 'on', previousState);
        console.error('[uBR] hostname switch failed:', error);
    } finally {
        dom.cl.remove(target, 'hnSwitchBusy');
    }
};
...
const pollForContentChange = (( ) => {
    const pollCallback = async function() {
        ...
        const changed = response === true || response?.changed === true;
        if ( changed ) {
            await getPopupData(popupData.tabId);
            return;
        }
...

{
    ...
    // The logger embeds the popup in a narrow iframe. Apply its orientation
    // before any content is measured so expanded sections stack vertically.
    ...
    const setOrientation = async ( ) => {
        ...
        if ( window.innerWidth < minWidth ) {
            ...
            popupRoot.classList.remove('portrait');
        }
    };
