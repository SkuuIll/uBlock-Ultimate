        ...
        datasetPromise = io.fetchText(datasetURL).then(details => {
            ...
            if ( details.content.startsWith('[') ) {
                try {
                    ...
                    console.warn('[uBR] benchmarks: JSON parse failed', e);
                }
            } else {
                ...
                while ( lineIter.eot() === false ) {
                    ...
                    try {
                        ...
                        console.warn('[uBR] benchmarks: line JSON parse failed', e);
                        parsed.length = 0;
                        break;
                    }
                }
...
export async function tokenHistogramsfunction() {
    ...
    for ( let i = 0; i < requests.length; i++ ) {
        ...
        fctxt.setType(request.cpt);
        const r = sfne.matchRequest(fctxt);
        for ( const [ keyword ] of request.url.toLowerCase().matchAll(reTokens) ) {
