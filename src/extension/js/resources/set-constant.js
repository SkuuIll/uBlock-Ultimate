...
export function validateConstantFn(trusted, raw, extraArgs = {}) {
    ...
    if ( raw === 'undefined' ) {
        ...
        if ( raw.startsWith('json:') ) {
            try { value = safe.JSON_parse(raw.slice(5)); } catch (e) { console.warn('[uBlock Ultimate] set-constant: JSON_parse failed', e); return; }
        } else if ( raw.startsWith('{') && raw.endsWith('}') ) {
            try { value = safe.JSON_parse(raw).value; } catch (e) { console.warn('[uBlock Ultimate] set-constant: JSON_parse value failed', e); return; }
        }
    } else {
        return;
    }
