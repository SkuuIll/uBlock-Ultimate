...
function setTheme(theme, propagate = false) {
    ...
    for (;;) {
        ...
        if ( w === w.parent ) { break; }
        w = w.parent;
        try { void w.document; } catch (e) { console.warn('[uBlock Ultimate] theme: cross-origin frame access failed', e); return; }
    }
}
...
function setAccentColor(
    ...
    for (;;) {
        ...
        if ( w === w.parent ) { break; }
        w = w.parent;
        try { void w.document; } catch (e) { console.warn('[uBlock Ultimate] theme: cross-origin frame access in setAccentColor failed', e); break; }
    }
}

{
    ...
    vAPI.messaging.send('dom', { what: 'uiStyles' }).then(response => {
        ...
        console.warn('[uBlock Ultimate] theme: uiStyles failed', e);
    });
