...
vAPI.generateSecret = (size = 1) => {
    ...
    while ( size-- ) {
        ...
        secret += (arr[0] % 2176782336 + 2176782336).toString(36).slice(1);
    }
    return secret;
};
...
vAPI.storage = {
    get(key, ...args) {
        return webext.storage.local.get(key, ...args).catch(reason => {
            console.warn('[uBR] vapi-background: storage.local.get failed', reason);
        });
    },
    set(...args) {
        return webext.storage.local.set(...args).catch(reason => {
            console.warn('[uBR] vapi-background: storage.local.set failed', reason);
        });
    },
    remove(...args) {
        return webext.storage.local.remove(...args).catch(reason => {
            console.warn('[uBR] vapi-background: storage.local.remove failed', reason);
        });
    },
    clear(...args) {
        return webext.storage.local.clear(...args).catch(reason => {
            console.warn('[uBR] vapi-background: storage.local.clear failed', reason);
        });
    },
...
if ( webext.storage.local.getBytesInUse instanceof Function ) {
    vAPI.storage.getBytesInUse = function(...args) {
        return webext.storage.local.getBytesInUse(...args).catch(reason => {
            console.warn('[uBR] vapi-background: storage.local.getBytesInUse failed', reason);
        });
    };
}
...
vAPI.Tabs = class {
    ...
    async executeScript(...args) {
        ...
        if ( scriptArgs.code && typeof webext.tabs?.executeScript === 'function' ) {
            try {
                ...
                console.warn('[uBR] vapi-background: tabs.executeScript failed', e);
            }
        ...
        if ( typeof browser.scripting !== 'undefined' ) {
            ...
            try {
                ...
                if ( scriptArgs.allFrames === true ) {
                    target.allFrames = true;
                }
                
                const execOptions = {
                    target: target,
                    files: [scriptArgs.file],
                };
                ...
                console.warn('[uBR] vapi-background: browser.scripting.executeScript failed', e);
            }
        } else if ( typeof webext.tabs?.executeScript === 'function' ) {
            try {
                result = await webext.tabs.executeScript(...args);
            } catch (e) {
                console.warn('[uBR] vapi-background: tabs.executeScript fallback failed', e);
            }
        }
        return Array.isArray(result) ? result : [];
    }
    ...
    async get(tabId) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.get failed for tab', tabId, e);
        }
    ...
    async insertCSS(tabId, details) {
        if ( typeof browser.scripting !== 'undefined' ) {
            ...
            if ( typeof details.frameId === 'number' ) {
                options.target.frameIds = [ details.frameId ];
            } else if ( details.allFrames === true ) {
                options.target.allFrames = true;
            }
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: browser.scripting.insertCSS failed', e);
            }
        }
        if ( vAPI.supportsUserStylesheets ) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.insertCSS failed', e);
        }
    }
    ...
    async query(queryInfo) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.query failed', e);
        }
    ...
    async removeCSS(tabId, details) {
        if ( typeof browser.scripting !== 'undefined' ) {
            ...
            try {
                await browser.scripting.removeCSS(options);
                return;
            } catch (e) {
                console.warn('[uBR] vapi-background: scripting.removeCSS error:', e);
            }
        }
        if ( vAPI.supportsUserStylesheets ) {
            details.cssOrigin = 'user';
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: webext.tabs.removeCSS failed', e);
        }
    }
    ...
    async update() {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.update failed', e);
        }
        return tab instanceof Object ? tab : null;
    }
    ...
    async remove(tabId) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.remove failed for tab', tabId, e);
        }
    }
    ...
    async reload(tabId, bypassCache = false) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: tabs.reload failed for tab', tabId, e);
        }
    }
    ...
    sanitizeURL(url) {
        ...
        return s.replace(/\s+/g, '') + url.slice(pos);
    }
    ...
    vAPI.windows = {
        get: async function() {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: windows.get failed', e);
            }
        ...
        create: async function() {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: windows.create failed', e);
            }
            return win instanceof Object ? win : null;
        },
        update: async function() {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: windows.update failed', e);
            }
            return win instanceof Object ? win : null;
        },
    };
...
// Support both browserAction (MV2) and action (MV3)
// In Firefox MV3, browser.action is the correct API (not browser.browserAction)
// Check browser.action first (MV3), then fallback to browser.browserAction (MV2)
const browserActionAPI = 
    (typeof browser.action !== 'undefined' && browser.action instanceof Object ? browser.action : undefined) ||
    (typeof browser.browserAction !== 'undefined' && browser.browserAction instanceof Object ? browser.browserAction : undefined);

if ( browserActionAPI !== undefined ) {
    vAPI.browserAction = {
        setTitle: async function() {
            try {
                await browserActionAPI.setTitle(...arguments);
            }
            catch (e) {
                console.warn('[uBR] vapi-background: browserAction.setTitle failed', e);
            }
        },
    };
    // Not supported on Firefox for Android
    if ( browserActionAPI.setIcon ) {
        vAPI.browserAction.setBadgeTextColor = async function() {
            try {
                await browserActionAPI.setBadgeTextColor(...arguments);
            }
            catch (e) {
                console.warn('[uBR] vapi-background: browserAction.setBadgeTextColor failed', e);
            }
        };
        vAPI.browserAction.setBadgeBackgroundColor = async function() {
            try {
                await browserActionAPI.setBadgeBackgroundColor(...arguments);
            }
            catch (e) {
                console.warn('[uBR] vapi-background: browserAction.setBadgeBackgroundColor failed', e);
            }
        };
        vAPI.browserAction.setBadgeText = async function() {
            try {
                await browserActionAPI.setBadgeText(...arguments);
            }
            catch (e) {
                console.warn('[uBR] vapi-background: browserAction.setBadgeText failed', e);
            }
        };
        vAPI.browserAction.setIcon = async function() {
            try {
                await browserActionAPI.setIcon(...arguments);
            }
            catch (e) {
                console.warn('[uBR] vapi-background: browserAction.setIcon failed', e);
            }
        };
    }
}
    ...
    (( ) => {
        ...
        const safeGetImageData = function(ctx, w, h) {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: getImageData failed', e);
            }
            return data;
        };
...
// Support both browserAction (MV2) and action (MV3) for onClicked
const browserActionOnClicked = browser.browserAction?.onClicked || browser.action?.onClicked;
if ( browserActionOnClicked instanceof Object ) {
    browserActionOnClicked.addListener((tab) => {
        vAPI.tabs.open({
            select: true,
    ...
    CallbackWrapper: class {
        ...
        proxy(response) {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: port.postMessage failed', e);
...
vAPI.contextMenu = webext.menus && {
    ...
    setEntries: function(entries, callback) {
        ...
        if ( n !== 0 && callback !== null ) {
            if ( this._callback !== null && this._callback !== callback ) {
...
vAPI.adminStorage = (( ) => {
    ...
    const cacheManagedStorage = async ( ) => {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: storage.managed.get failed', e);
        }
    ...
    return {
        get: async function(key) {
            ...
            try {
                ...
                console.warn('[uBR] vapi-background: managed storage.get failed', e);
                bin = {};
            }
            cacheManagedStorage();
...
vAPI.cloud = (( ) => {
    ...
    const push = async function(details) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: deleteChunks failed', e);
        }
    ...
    const pull = async function(details) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: cloud storage decode failed', e);
        }
        return entry;
    };
    ...
    const used = async function(datakey) {
        ...
        try {
            ...
            console.warn('[uBR] vapi-background: storage.sync.getBytesInUse failed', e);
        }
